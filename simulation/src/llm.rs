...
//! 決定論的 socsim コアからは [`WarClient`] 型エイリアス経由でのみ触れる．
//!
...
//! クレット (issue #21/#26) に標準化したため `reqwest` / `sha2` は使わず，HTTP と
//! ハッシュは socsim-llm が所有する．
//!
//! テストでは `socsim-llm::mock::ScriptedClient` を `Box<dyn LlmClient>` として
//! 同じ [`WarClient`] に流し込める．`socsim-llm` が `Box<dyn LlmClient>` に
...
pub type WarClient = CachingClient<Box<dyn LlmClient>>;
...
///   `gpt-4`/`claude-2`)．未設定なら空キーのフォールバックを置く (Ollama が
///   成功すれば呼ばれない; 両方失敗時のみ設定エラーになる)．
/// - キャッシュ: `settings.cache_path` があればその JSON ファイル，なければ
///   in-memory．
pub fn build_live_client(settings: &LlmSettings) -> Result<WarClient, LlmError> {
    ...
    // 本ラッパは OLLAMA_MODEL 既定の上書きと replication 固有の `LlmSettings`
    // (cache_path) を受け取る薄い層．
    socsim_llm::build_live_client(settings.cache_path.as_deref().map(Path::new))
}

/// 任意の [`LlmClient`] (例: `mock::ScriptedClient`) をキャッシュで包んだ
/// [`WarClient`] を作る (主にテスト用)．
pub fn wrap_client<C: LlmClient + 'static>(backend: C, cache: PromptCache) -> WarClient {
